...
export const commands: Chat.Commands = {
  ...
  // Disconnect with random message
  d: 'poof',
  cpoof: 'poof',
  ...
  // Help for seen command
  seenhelp: [`/seen [user] - Shows when the user last connected on the server.`],

  // Display miscellaneous command help
  mischelp(target, room, user) {
    if (!this.runBroadcast()) return;
    this.sendReplyBox(
      `<details><summary><b><center>Miscellaneous Commands</center></b></summary>` +
  ...
  impulsehelp(target, room, user) {
    if (!this.runBroadcast()) return;
    this.sendReplyBox(
      `<details><summary><b><center>Impulse Server Commands</center></b></summary>` +
